...
import { bringToFront, toggleCollapse, closePanel, updatePanelGeometry } from '../hooks/state.js';
...
export function createPanel(panelState, contentRenderer) {
  ...
  // Bind Mouse events for dragging header
  header.addEventListener('mousedown', (e) => {
    // Prevent dragging on control button clicks
    ...
    ghost.style.transform = `translate(${snapToGrid(startLeft)}px, ${snapToGrid(startTop)}px)`;
    ...
    function onMouseMove(moveEvent) {
      ...
      ghost.style.transform = `translate(${snapToGrid(finalLeft)}px, ${snapToGrid(finalTop)}px)`;
    }
    ...
    function onMouseUp() {
      ...
      const snappedLeft = snapToGrid(finalLeft);
      const snappedTop = snapToGrid(finalTop);
  ...
  if (!isCollapsed) {
    ...
    directions.forEach((dir) => {
      ...
      handle.addEventListener('mousedown', (e) => {
        ...
        ghost.className = 'panel-snap-ghost';
        ghost.style.transform = `translate(${snapToGrid(startLeft)}px, ${snapToGrid(startTop)}px)`;
        ghost.style.width = `${snapToGrid(startW)}px`;
        ghost.style.height = `${snapToGrid(startH)}px`;
        ghost.style.zIndex = zIndex - 1;
        container.parentElement.appendChild(ghost);
        ...
        function onMouseMove(moveEvent) {
          ...
          ghost.style.transform = `translate(${snapToGrid(finalLeft)}px, ${snapToGrid(finalTop)}px)`;
          ghost.style.width = `${snapToGrid(finalW)}px`;
          ghost.style.height = `${snapToGrid(finalH)}px`;
        }
        ...
        function onMouseUp() {
          ...
          updatePanelGeometry(id, {
            x: snapToGrid(finalLeft),
            y: snapToGrid(finalTop),
            width: snapToGrid(finalW),
            height: snapToGrid(finalH),
          });
        }
